...
`run_live_trading.py` 預設只做 dry-run。下單使用 CCXT 統一交易對格式，預設為 Binance／OKX USDT 永續：`BTC/USDT:USDT`。
...
目前 adapter 會處理交易數量與價格精度、槓桿、reduce-only 保護單、同方向持倉去重與反方向持倉衝突；實盤前仍應先以 testnet 驗證交易所帳戶模式、觸發單格式與最小下單量。
