...
class MultiTimeframeBacktester:
    ...
    def run_backtest(self, mode="NONE", enable_be=True, tp1_close_pct=0.5, be_trigger_ratio=1.0):
        ...
        for i in range(lookback, n_rows):
            ...
            if active_position is not None:
                ...
                original_sl = active_position["original_sl"] 
                ...
                if pos_type == "LONG":
                    ...
                    if is_sl_hit and is_tp1_hit_now:
                        first_hit = self._check_1m_sequence(prev_time, curr_time, current_sl, tp1, a_is_low=True, b_is_high=True)
                        ...
                        exit_price = current_sl
                        pnl = ((tp1_close_pct * size * (tp1 - entry_price) + (1.0 - tp1_close_pct) * size * (exit_price - entry_price))
                               if tp1_hit else size * (exit_price - entry_price))
                        balance += pnl
                        
                        # 👈 核心修正：只有在啟用並真正觸發 be_active，或平保不虧時，result 才叫 BREAKEVEN
                        if be_active or (tp1_hit and exit_price >= entry_price):
                            res_tag = "BREAKEVEN"
                        else:
                            res_tag = "STOP_LOSS"
                        ...
                        trades.append({
                            "type": "LONG", "entry_time": active_position["entry_time"], "exit_time": curr_time,
                            "pnl": pnl, "pnl_pct": (pnl / active_position["entry_balance"]) * 100,
                            "result": res_tag, "rr_potential": rr_potential, "rr_realized": pnl / initial_risk_usd
                        })
                        active_position = None
                    elif is_tp1_hit_now:
                        active_position["tp1_hit"] = True
                    ...
                    if is_sl_hit and is_tp1_hit_now:
                        first_hit = self._check_1m_sequence(prev_time, curr_time, current_sl, tp1, a_is_low=False, b_is_high=False)
                        ...
                        exit_price = current_sl
                        pnl = ((tp1_close_pct * size * (entry_price - tp1) + (1.0 - tp1_close_pct) * size * (entry_price - exit_price))
                               if tp1_hit else size * (entry_price - exit_price))
                        balance += pnl
            ...
            if active_position is None:
                ...
                if allowed_long and fvg_overlap_long and pd.notna(h1_top):
                    if (prev["close"] > prev["VWAP"]) and (prev["local_bias"] == "LONG"):
                        ...
                        if curr["low"] <= entry_p:
                            if prev["close"] > sl and tp1 > entry_p:
        ...
        if total_trades == 0:
            return {"trades": 0, "win_rate": 0.0, "profit": 0.0, "balance": balance, "tp_all": 0, "be": 0, "sl": 0, "avg_potential_rr": 0.0, "avg_realized_rr": 0.0, "missed": missed_trades_count}
        ...
        # ---------------------------------------------------------------------------
        # 🛠️ 核心升級：專業機構級「純粹賺賠比」統計引擎
        # ---------------------------------------------------------------------------
        df_profitable = df_trades[df_trades["pnl"] > 0]  # 正報酬賺錢單
        df_losing = df_trades[df_trades["pnl"] < 0]      # 負報酬虧損單
        ...
        return {
            ...
            "avg_realized_rr": pure_realized_rr, # 👈 注入真正不受複利噪音污染的純賺賠比
            "missed": missed_trades_count  
        }
...
if __name__ == "__main__":
    try:
        ...
        experiments = [
            {
                "name": "15m+1H 基礎版 (完全不保本，生死看淡)", 
                "key": "NONE", "enable_be": False, "tp1_close_pct": 0.0, "be_ratio": 1.0
            },
            {
                "name": "15m+1H 輕度保護 (TP1只減倉20%, 不主動推保本)", 
                "key": "NONE", "enable_be": False, "tp1_close_pct": 0.2, "be_ratio": 1.0
            },
            {
                "name": "15m+1H+4H 三重共振 (完全不保本，利潤最大化)", 
                "key": "4H", "enable_be": False, "tp1_close_pct": 0.0, "be_ratio": 1.0
            },
            {
                "name": "15m+1H+4H 三重共振 (微調：走一半距離提早推保本+減倉30%)", 
                "key": "4H", "enable_be": True, "tp1_close_pct": 0.3, "be_ratio": 0.5
            },
        ]
        ...
        for exp in experiments:
            res = backtester.run_backtest(
                mode=exp["key"], 
                enable_be=exp["enable_be"], 
                tp1_close_pct=exp["tp1_close_pct"], 
                be_trigger_ratio=exp["be_ratio"]
            )
            results.append({
                "實驗策略組合": exp["name"], "實質交易": res["trades"], 
                "預期盈虧比": f"1 : {res['avg_potential_rr']:.2f}", 
                "實質純賺賠比": f"1 : {res['avg_realized_rr']:.2f}", # 👈 乾淨回顯
                "勝率(正報酬)": f"{res['win_rate']:.2f}%", 
                "吃滿(TP2)": res["tp_all"], "保本(BE)": res["be"], "滿額止損(SL)": res["sl"],
        ...
        print("\n" + "═"*145)
        print(" 📊  V8 全知之眼 - 策略出場控制參數對比回測報告 (真實純粹賺賠比引擎)")
        print("═"*145)
