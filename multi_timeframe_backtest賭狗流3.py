...
# ---------------------------------------------------------------------------
# V8 全知之眼 - 防污染、完全實盤對齊運算內核（升級版：修正四大回測陷阱）
# ---------------------------------------------------------------------------
class V8_Omniscient_Eye:
    ...
    def calculate_v8_indicators(self):
        ...
        # 🛠️ 陷阱修正 1：計算日內每根 K 線距離開盤的序號（用以排除清晨統計不足的雜訊）
        # 15m 級別，1天有 96 根。我們利用 cumcount 找出當天第幾根 K 線
        ...
        # 🛠️ 陷阱修正 2：清晨冷啟動過濾 (排除每天前 16 根 15m K 線，即前 4 小時籌碼未熱身階段)
        # 防止清晨標準差過窄導致保本(BE)或 TP1 被隨機雜訊隨便掃掉
...
class MultiTimeframeBacktester:
    ...
    def _check_1m_sequence(self, start_time, end_time, target_a, target_b, a_is_low, b_is_high):
        ...
        for _, row in m1_slice.iterrows():
            ...
            if hit_a and hit_b: return 'A' # 🛠️ 陷阱修正 3：同分鐘生死局，極端防禦一律算先吃止損
            if hit_a: return 'A'
            if hit_b: return 'B'
        return 'NONE'
    ...
    def run_backtest(self, mode="NONE", enable_be=False, tp1_close_pct=0.0, be_trigger_ratio=1.0):
        ...
        for i in range(lookback, n_rows):
            ...
            if active_position is None:
                ...
                # 🛠️ 陷阱修正 2.1：如果清晨指標尚未冷啟動完成（為 NaN），直接跳過交易
                ...
                if allowed_long and fvg_overlap_long and pd.notna(h1_top):
                    if (prev["close"] > prev["VWAP"]) and (prev["local_bias"] == "LONG"):
                        # 🛠️ 陷阱修正 4：防空降滑價。如果當前 K 線開盤價已經跳空跌破原本想進場的 h1_top，
                        # 實盤中你會在更差的開盤價成交，因此進場價取 min(curr["open"], h1_top)
                ...
                # 🔴 空頭進場
                    ...
                    if (prev["close"] < prev["VWAP"]) and (prev["local_bias"] == "SHORT"):
                        # 🛠️ 陷阱修正 4：防空降滑價。空頭取 max(curr["open"], h1_bottom)
        ...
        # 🛠️ 陷阱修正 5：未平倉合約強制清算（防止隱藏大回撤與倖存者偏差）
        if active_position is not None:
            last_bar = df.iloc[-1]
    ...
    def analyze_results(self, trades):
        ...
        # 統計加入強制平倉的計數，將其算在吃滿或保本的延伸中（或依實質正負報酬歸類）
        ...
        force_count = len(df_trades[df_trades["result"] == "FORCE_CLOSE"])
    ...
    def generate_and_print_report(self, strategy_name, trades, missed_count):
        ...
        print("\n" + "═"*145)
        print(f" 📊  V8 全知之眼 [陷阱全面修正版] - 策略：【{strategy_name}】跨月碎裂化戰報")
        print("═"*145)
